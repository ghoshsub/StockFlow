...
/**
 * Brand entity — represents a product brand in the StockFlow system.
 *
 * Stored in the `brands` table.
...
public class Brand extends BaseEntity {
    ...
    /** Brand name — must be unique and between 2-100 characters. */
    @NotBlank
    @Size(min = 2, max = 100)
    @Column(name = "name", nullable = false, unique = true, length = 100)
    private String name;

    /** Optional human-readable description of the brand. */
    @Size(max = 255)
    ...
    /**
     * Soft-delete flag.
     * true  = brand is active and visible.
     * false = brand has been "deleted" (hidden from normal queries).
